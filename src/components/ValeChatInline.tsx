...
export default function ValeChatInline({ listingId }: { listingId: string }) {
  ...
  const [hasMyHome, setHasMyHome] = useState(false);
  const [myHomeAddress, setMyHomeAddress] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (phone) {
      setHasMyHome(true);
      fetch(`${IDX_API}/api/idx/myhome/profile/${encodeURIComponent(phone)}`)
        .then(r => r.ok ? r.json() : null)
        .then(d => {
          const addr = d?.profile?.address || d?.address;
          if (addr) setMyHomeAddress(addr);
        })
        .catch(() => {});
    }
  }, []);
      ...
      <div className="relative bg-white rounded-2xl overflow-hidden">
        ...
        <div className="px-4 pt-3 pb-1 space-y-2">
          ...
          {hasMyHome && (
            <button
              onClick={() => {
                const addr = myHomeAddress || "my home";
                send(`Compare this property to my home at ${addr}`);
                chatContainerRef.current?.closest("[data-vale-widget]")?.scrollIntoView({ behavior: "smooth", block: "center" });
              }}
              disabled={loading || !sessionId}
              className="w-full flex items-center gap-2 rounded-xl border-2 border-gold/40 bg-gold/5 px-4 py-2 text-sm font-semibold text-navy hover:bg-gold/15 transition-all disabled:opacity-40"
            >
